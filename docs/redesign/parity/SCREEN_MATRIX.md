...
| A4 | `vTargets` | 465–525 | `/targets` `Targets.tsx` | Implemented · dark and light checked on staging | §T. Checked at 1536 CSS px, dark; element sizes measured. Pending: 1280/1440, light, a save and a confirm exercised |
...
| A6 | `vSettings` | 598–786 + script 3515 | `/settings` `Settings.tsx`, `DataPanel.tsx` | Team, Shopify, Historical and Brand codes implemented; Historical checked on staging (dark); Reference mapped | Rail, Team (staff, invite row, pending invitations with Resend) and Appearance switch done. Shopify and sync: store card, Connection card, operations behind *Technical detail*; *Read the catalogue* stands in for the export's *Refresh now*, which has no single backend act. Historical setup now has the export's four columns — the state says whether her terms reach back to when she started, off a new `earliest_terms_month` on the roster, and links to the months that need writing. Its bulk *Review months from January* button is still absent: no such operation exists, and a button that reviewed nothing would be worse than none. Reference: title renamed *Policy in force*; the version editor and activity filter are kept |
...
|---|---|---|---|---|---|
| B1 | `vModel` | 787–1059 + script 2870 | `/affiliates/:id` `AffiliateDetail.tsx` | Implemented · application panel checked on staging (dark) | Application panel rebuilt to the export's two columns — *Submitted information* beside *Setup before approval*, the approval under the second; her six answers were two taps away under Overview before. Hero, section tabs with the month always beside them, Overview's three figures and *Current terms*; Wardrobe as received pieces and a not-yet list; Performance as her orders (new `GET /api/affiliates/{id}/orders/{month}`); Targets and Payments as month-by-month tables (new `GET /api/affiliates/{id}/record`). Kept: code verification, start month, shipping edit, codes form, corrections panel, rules preview. The embedded Targets editing grid is replaced by the export's read-only table and a link to Targets |
| B2 | `vInvite` | 1232–1279 | `/affiliates/invite` `InvitePage.tsx` | Implemented, not yet seen on staging | A screen now, not a modal: invite card with the server's link and Copy, *Applications awaiting review*, *Outstanding invitations* with Resend and Withdraw. `InviteModel.tsx` deleted |
